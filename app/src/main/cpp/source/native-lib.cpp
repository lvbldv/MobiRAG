...
Java_com_mobirag_LlamaCppWrapper_loadModel(JNIEnv* env, jobject, jstring modelPath) {
...
//    };
    ...
    ctx_params.n_ctx = 30000;
    ctx_params.n_batch = 30000;
...
Java_com_mobirag_LlamaCppWrapper_runInference(JNIEnv* env, jobject, jstring prompt) {
    std::lock_guard<std::mutex> lock(llama_state.mutex);
    ...
    if (newLen < 0) {
        ...
        return env->NewStringUTF("Error: Failed to format prompt");
    }

    std::string _prompt(_formattedMessages.begin(), _formattedMessages.begin() + newLen);
    auto vocab = llama_model_get_vocab(llama_state.model);

    try {
        std::vector<llama_token> tokens = common_tokenize(vocab, _prompt, true, true);
        if (tokens.size() > llama_n_ctx(llama_state.ctx)) {
            throw std::runtime_error("Prompt exceeds context size");
        }
        llama_batch batch = llama_batch_init((int)tokens.size() + 32, 0, 1);
...
//        }
        ...
        for (int i = 0; ; ++i) {
            ...
            if (!logits) {
                throw std::runtime_error("Null logits during generation");
            }
...
Java_com_mobirag_LlamaCppWrapper_freeModel(JNIEnv*, jobject) {
    std::lock_guard<std::mutex> lock(llama_state.mutex);
    if (llama_state.ctx) {
    ...
    if (llama_state.sampler) {
        llama_sampler_free(llama_state.sampler);
        llama_state.sampler = nullptr;
    }
}

extern "C" JNIEXPORT void JNICALL
Java_com_mobirag_LlamaCppWrapper_runInferenceStream(
        JNIEnv* env, jobject, jstring jPrompt, jobject listener) {
    ...
    if (!llama_state.ctx || !llama_state.model) {
        ...
        env->CallVoidMethod(listener, onError, env->NewStringUTF("Model not loaded"));
        return;
    }

    const char* input = env->GetStringUTFChars(jPrompt, nullptr);
    std::vector<char> formatted = std::vector<char>(llama_n_ctx(llama_state.ctx));
    llama_state.messages.clear();
    llama_state.messages.push_back({strdup("system"), strdup("You are a helpful assistant.")});
    llama_state.messages.push_back({strdup("user"), strdup(input)});

    int newLen = llama_chat_apply_template(nullptr, llama_state.messages.data(), llama_state.messages.size(), true,
                                           formatted.data(), formatted.size());
    if (newLen > (int)formatted.size()) {
        formatted.resize(newLen);
        newLen = llama_chat_apply_template(nullptr, llama_state.messages.data(), llama_state.messages.size(), true,
                                           formatted.data(), formatted.size());
    }

    if (newLen < 0) {
        env->ReleaseStringUTFChars(jPrompt, input);
        jclass cls = env->GetObjectClass(listener);
        jmethodID onError = env->GetMethodID(cls, "onError", "(Ljava/lang/String;)V");
        env->CallVoidMethod(listener, onError, env->NewStringUTF("Error formatting prompt"));
        return;
    }

    std::string prompt(formatted.begin(), formatted.begin() + newLen);
    auto vocab = llama_model_get_vocab(llama_state.model);
    ...
    try {
        ...
        jclass cls = env->GetObjectClass(listener);
        jmethodID onToken = env->GetMethodID(cls, "onTokenGenerated", "(Ljava/lang/String;)V");
        jmethodID onComplete = env->GetMethodID(cls, "onComplete", "()V");

        int n_past = tokens.size();
        for (;;) {
            const float* logits = llama_get_logits(llama_state.ctx);
            if (!logits) throw std::runtime_error("Null logits during generation");
            ...
            env->CallVoidMethod(listener, onToken, tokenStr);
        ...
        jclass cls = env->GetObjectClass(listener);
        jmethodID onError = env->GetMethodID(cls, "onError", "(Ljava/lang/String;)V");
        env->CallVoidMethod(listener, onError, env->NewStringUTF(e.what()));
    }
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_mobirag_SentenceEmbeddingActivity_compressEmbeddingWithPQ(
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_mobirag_SentenceEmbeddingActivity_compressEmbeddingsWithPQ(
    ...
    // === Create the PQ index ===
    const int numSubvectors = 48;   // Number of subquantizers (m)
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_mobirag_SentenceEmbeddingActivity_searchIndexNative(
    ...
    try {
        ...
        std::vector<int64_t> validResults;
        for (int i = 0; i < top_k; i++) {
            float dis_i = distances[i];
            if (dis_i < distanceThreshold) {
                validResults.push_back(indices[i]);
            }
        }
        ...
        for (int i = 0; i < validResults.size(); i++) {
            resultJson += std::to_string(validResults[i]);
            if (i < validResults.size() - 1) resultJson += ",";
        }
        resultJson += "]";
...
#define JNIREG_CLASS_BASE "com/mobirag/MainActivity"
...
static JNINativeMethod gMethods_SentenceEmbedding[] = {
        {"compressEmbeddingsWithPQ",
         ...
         (void *) Java_com_mobirag_SentenceEmbeddingActivity_compressEmbeddingsWithPQ},
        {"compressEmbeddingWithPQ",
         "([FLjava/lang/String;)Ljava/lang/String;",
         (void *) Java_com_mobirag_SentenceEmbeddingActivity_compressEmbeddingWithPQ},
        {"searchIndexNative",
                "([FILjava/lang/String;)Ljava/lang/String;",
                (void *) Java_com_mobirag_SentenceEmbeddingActivity_searchIndexNative}
};
...
static int registerNatives(JNIEnv *env) {
    ...
    if (!registerNativeMethods(env, "com/mobirag/SentenceEmbeddingActivity",
